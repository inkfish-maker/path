...
// 路径插值
void path_spline(vector<PointType> &points_way, vector<PointType> &way_splined)
{
    //清除插值后的路径
    way_splined.clear();
    //如果原最优路径数量小于等于1
    if (points_way.size() <= 1)
    {
        return;
    }
    // std::cout<<"splined"<<std::endl;
    std::vector<DataType> s_list, x_list, y_list;
    s_list.push_back(0);
    // std::cout<<"points_way.size() = "<<points_way.size()<<std::endl;
    for (int i = 0; i < points_way.size(); ++i)
    {
        // std::cout<<"points_way["<<i<<"]="<<points_way[i]<<std::endl;
        x_list.push_back(points_way[i].x);
        y_list.push_back(points_way[i].y);
        if (i != 0)
        {
            DataType dist = sqrt(pow(fabs(points_way[i].x - points_way[i - 1].x), 2) + pow(fabs(points_way[i].y - points_way[i - 1].y), 2));
            dist += s_list[i - 1];
    ...
    DataType total_len = s_list[s_list.size() - 1];
    for (DataType len = 0; len + EPS < total_len; len += sub_len)
    {
        PointType p;
        p.x = spline_x(len);
        p.y = spline_y(len);
        way_splined.push_back(p);
    }
    return;
}
...
void run(int start_tri, int p1_index, int p2_index)
{
    ...
    while (true)
    {
        ...
        std::vector<MyPoint> way_splined;
        path_spline(paths[pathindex], way_splined);
        // std::cout << paths[pathindex].size() << "----" << way_splined.size() << std::endl;
        ...
        for (int i = 0; i < way_splined.size() - 1; i++)
        {
            line(temp, Point2d(way_splined[i].x, way_splined[i].y), Point2d(way_splined[i + 1].x, way_splined[i + 1].y), Scalar(0, 255, 0));
        }
        end_c = clock();
