...
{
    fstream fin;
    fin.open(WORK_SPACE_PATH + "/path_datas/path4.txt");
    int numberPoints, color;
    double x, y;
    char none_key;
    fin >> numberPoints;
    bool repeat_flag = false;
    for (int i = 0; i < numberPoints;)
    {
        fin >> x;
        fin >> none_key;
        fin >> y;
        ...
        PointType p(x, y);
        //检查重复点
        for (vector<PointType>::iterator ptr = points.begin(); ptr != points.end(); ptr++)
        {
...
void show_tri(vector<Triangle> &triangles_)
{
    for (int i = 0; i < triangles.size(); i++)
    {
        ...
        // Mat temp;
        // board.copyTo(temp);
        // line(temp, triangles_[i].p1, triangles_[i].p2, Scalar(0, 255, 0));
        // line(temp, triangles_[i].p1, triangles_[i].p3, Scalar(0, 255, 0));
        // line(temp, triangles_[i].p2, triangles_[i].p3, Scalar(0, 255, 0));
...
void Delaunay()
{
    ...
    for (int index = 0; index < point_num; index++)
    {
        ...
        {
            if (now_point == temp_edges[i].px || now_point == temp_edges[i].py)
                continue;
